...
class FFPLAY():
    ...
    def on_release(self,key):
        if self.active_media!="picture":return
        try:
    ...
    def play_images(self,file,key):# Images/Photos etc...
        if self.next_ready:
            ...
            while self.listener.running and self.key_now!=None :
                try:
                    stop_btn.configure(background="#bcbcbc",foreground="#ffffff")# Disabled
                    title_lbl.configure(text=f"Archivo de Foto: {os.path.basename(self.Media_Dict[str(self.key_now)])}")
    ...
    def on_complete(self,stream,file_path):
        ...
        msg4=f"{file_name} Fue Agregado a la Biblioteca {self.youtube_db}."
    ...
    def download_from_youtube(self,type):
        ...
        if not os.path.exists(save_folder):
            msg1=f"El Archivo {type} se Guardará en {save_folder}."
        ...
        try:
            ...
            msg1=f'¡Error al Descargar a {save_folder}!\n'
            msg2=f"Error: '{e}'"
            msg=msg1+msg2
            messagebox.showerror(title, msg)
    ...
    def load_library(self,db,skip=None):
        ...
        if len(self.Media_Dict)==0:
            ...
            return
        root.title(f"Mi Reproductor Multimedia ({db.replace("_"," ")} Biblioteca)")
