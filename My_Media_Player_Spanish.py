...
class FFPLAY():
    ...
    def load_menubar(self):
        ...
        upload_music.add_command(label="Cargar Carpeta a la Biblioteca Música",command=lambda:self.find_in_folder("Music"))
        upload_music.add_separator()
        upload_music.add_command(label="Subir Archivo/s a la Biblioteca Música",command=lambda:self.add_files_to_db("Music"))
        upload_music.add_separator()
        upload_music.add_command(label="Descargar Música de Youtube a la Biblioteca Música",command=lambda:self.download_from_youtube("Music"))
        upload_music.add_separator()
        ...
        favorite_music.add_command(label="Subir Carpeta a la Biblioteca Música Favorita",command=lambda:self.find_in_folder("Music_Favorite"))
        favorite_music.add_separator()
        ...
        favorite_music.add_separator()
        favorite_music.add_command(label="Descargar Música de Youtube a la Biblioteca Música Favorita",command=lambda:self.download_from_youtube("Music_Favorite"))
        favorite_music.add_separator()
        ...
        upload_videos.add_command(label="Cargar Carpeta a la Videoteca",command=lambda:self.find_in_folder("Videos"))
        upload_videos.add_separator()
        upload_videos.add_command(label="Subir Archivo/s a la Videoteca",command=lambda:self.add_files_to_db("Videos"))
        upload_videos.add_separator()
        upload_videos.add_command(label="Descargar Video de Youtube a la Videoteca",command=lambda:self.download_from_youtube("Videos"))
        upload_videos.add_separator()
        ...
        favorite_videos.add_command(label="Cargar Carpeta a la Biblioteca Videos Favorita",command=lambda:self.find_in_folder("Videos_Favorite"))
        favorite_videos.add_separator()
        ...
        favorite_videos.add_separator()
        favorite_videos.add_command(label="Descargar Video de Youtube a la Biblioteca Videos Favorita",command=lambda:self.download_from_youtube("Videos_Favorite"))
        favorite_videos.add_separator()
        ...
        music_videos.add_command(label="Cargar Carpeta a la Biblioteca Videos Música",command=lambda:self.find_in_folder("Videos_Music"))
        music_videos.add_separator()
        ...
        music_videos.add_separator()
        music_videos.add_command(label="Descargar Video de Youtube a la Biblioteca Videos Música",command=lambda:self.download_from_youtube("Videos_Music"))
        music_videos.add_separator()
        ...
        karoake_videos.add_command(label="Subir Carpeta a la Biblioteca Videos Karaoke",command=lambda:self.find_in_folder("Videos_Karaoke"))
        karoake_videos.add_separator()
        ...
        karoake_videos.add_separator()
        karoake_videos.add_command(label="Descargar Video de Youtube a la Biblioteca Videos Karaoke",command=lambda:self.download_from_youtube("Videos_Karaoke"))
        karoake_videos.add_separator()
        ...
        upload_image.add_command(label="Subir Carpeta a la Fototeca",command=lambda:self.find_in_folder("Pictures"))
        upload_image.add_separator()
        ...
        family_image.add_command(label="Cargar Carpeta a la Fototeca Familiar",command=lambda:self.find_in_folder("Pictures_Family"))
        family_image.add_separator()
        ...
        favorite_image.add_command(label="Cargar Carpeta a la Fototeca Favorita",command=lambda:self.find_in_folder("Pictures_Favorite"))
        favorite_image.add_separator()
    ...
    def on_complete(self,path,name):
        ...
        msg1="¡Descarga de Youtube Completada con éxito!\n"
        msg2="¡Ahora Puedes Cerrar Youtube si lo Deseas!\n"
        msg3=f"File Saved To {path}.\n"
        msg4=f"{name} Fue Agregado a la Biblioteca {self.youtube_db}."
        msg=msg1+msg2+msg3+msg4
        messagebox.showinfo(title,msg)
        self.youtube_db=""
    def download_from_youtube(self,type):
        if type=="Music":yt_folder=self.music_folder
        elif type=="Music_Favorite":yt_folder=self.music_favorite_folder
        elif type=="Videos":yt_folder=self.video_folder
        elif type=="Videos_Family":yt_folder=self.video_family_folder
        elif type=="Videos_Favorite":yt_folder=self.video_favorite_folder
        elif type=="Videos_Karaoke":yt_folder=self.video_karaoke_folder
        elif type=="Videos_Music":yt_folder=self.video_music_folder
        else:return
        self.youtube_db=type
        if not os.path.exists(yt_folder):
            msg1=f"El Archivo {type} se Guardará en {yt_folder}."
            os.makedirs(yt_folder)
        title="<Descargas de Youtube>"
        msg1="¡Ingrese una URL de Youtube para Descargar!\n"
        msg2="Para Obtener una URL, 'Inicie Sesión' en su Cuenta\n"
        msg3="de Youtube y Reproduzca el Video o la Música que Desee.\n"
        msg4="Mientras Juega, haga Clic Derecho en la Ventana de\n"
        msg5="Reproducción y Seleccione Copiar URL. Después de esto,\n"
        msg6="Pausa la Reproducción pero deja Youtube abierto. A\n"
        msg7="Continuación, Pegue la URL Copiada en el Campo a\n"
        msg8="Continuación Usando las Teclas del Teclado ctrl + v."
        msg=msg1+msg2+msg3+msg4+msg5+msg6+msg7+msg8
        my_url=my_askstring(title,msg,"")
        if my_url==None:return
        try:
            yt=YouTube(url=my_url, on_progress_callback=on_progress)
            ys=yt.streams.get_highest_resolution()
            ys.download(output_path=yt_folder,filename=ys.default_filename)
        except Exception as e:
            msg1=f'¡Error al Descargar a {yt_folder}!\n'
            msg2=f"Error: '{e}'"
            msg=msg1+msg2
            messagebox.showerror(title, msg)
    ...
    def load_library(self,db,skip=None):
        ...
        if len(self.Media_Dict)==0:
            ...
            self.active_database=db
            root.title(f"Mi Reproductor Multimedia 5.1 ({db.replace("_"," ")} Biblioteca), Reproducción en un Dispositivo de Audio: {self.Active_Device}")
    ...
    def select_output_device(self,device):
        try:
            ...
            Level.set(5.0)
            self.Master_Volume.SetMasterVolumeLevelScalar(Level.get()/100, None)
            root.title(f"Mi Reproductor Multimedia 5.1 ({self.active_database.replace("_"," ")} Biblioteca), Reproducción en un Dispositivo de Audio: {self.Active_Device}")
        except Exception as ex:
...
if __name__ == '__main__':
    ...
    root.title("Mi Reproductor Multimedia 5.1")
