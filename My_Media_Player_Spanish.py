...
class FFPLAY():
    ...
    def on_release(self,key):
        if self.active_media!="image":return
        try:
    ...
    def play_images(self,file,key):# Images/Photos etc...
        if self.next_ready:
            ...
            while self.listener.running and self.key_now!=None :
                try:
                    stop_btn.configure(background="#bcbcbc",foreground="#ffffff")# Disabled
                    title_lbl.configure(text=f"Archivo de Imagen: {os.path.basename(self.Media_Dict[str(self.key_now)])}")
    ...
    def play_av(self,file,key):# Audio/Video Files
        if self.next_ready:
            ...
            if not self.ffplay_running:# Not Running
                ...
                stop_btn.configure(background="#bcbcbc",foreground="#ffffff")# Disabled
                self._reset_timer()
                title_lbl.configure(text=f"Dispositivo de Audio: {self.Active_Device}, {os.path.basename(self.Media_Dict[str(self.key_now)])}")
                try:
                    ...
                    window_x,window_y=self.set_window_coord(window_wid,window_hgt)
                    if self.active_media=="video":root.config(menu="")
    ...
    def _update_timer(self):
        ...
        if is_muted and Muted.get()==False:self.ctrl_btn_clicked(self,"mute")
        elif not is_muted and Muted.get()==True:self.ctrl_btn_clicked(self,"mute") 
        self._start_timer_thread()
    ...
    def rotate_image(self):
        ...
        msg1="Introduzca la Rotación de la Imagen en Grados.\n"
        msg2="¡El Rango es de -360 a 360 Grados!\n"
        msg3="Un Número Negativo gira la Imagen en el Sentido\n"
        msg4="de las Agujas del Reloj. Un Número Positivo hace girar\n"
        msg5="el Contador de Imágenes en el Sentido de las Agujas del Reloj."
        ...
        if angle!=None:
            try:
                ...
                title='<Rotación de Imágenes>'
                msg1='¡Error en la Rotación de la Imagen!\n'
    ...
    def remove_media_file(self,key=None,show_msg=None):
        try:
            ...
            title=f'<Eliminar Aarchivo {file_name}>'
            msg1=f'¡Error al Eliminar {file_name}!\n'
            msg2=f"Error: '{e}'"
            msg=msg1+msg2
            messagebox.showerror(title, msg)
        ...
        try:
            if len(self.Media_Dict)>0:
                ...
                if os.path.exists(file_to_move):
                    title=f'<Mover Archivo de Imagen a la Biblioteca de {to_db}>'
    ...
    def load_image_menu(self):
        ...
        self.menubar.add_cascade(label=' Imágenes ',menu=images_menu)
        images_menu.add_command(label='Rotar la Imagen y Guardarla',command=lambda:self.rotate_image())
        ...
        if self.active_database=="Images":
            move_image.add_command(label="Mover a la Bbiblioteca de Iimágenes Ffamiliares",command=lambda:self.move_image("Images_Family"))
            move_image.add_separator()
            move_image.add_command(label="Mover a la Biblioteca de Imágenes de Favoritos",command=lambda:self.move_image("Images_Favorite"))
        elif self.active_database=="Images_Family":
            move_image.add_command(label="Mover a la Biblioteca de Imágenes",command=lambda:self.move_image("Images"))
            move_image.add_separator()
            move_image.add_command(label="Mover a la Biblioteca de Imágenes de Favoritos",command=lambda:self.move_image("Images_Favorite"))
        elif self.active_database=="Images_Favorite":
            move_image.add_command(label="Mover a la Biblioteca de Imágenes",command=lambda:self.move_image("Images"))
            move_image.add_separator()
            move_image.add_command(label="Mover a la Biblioteca de Imágenes Familiares",command=lambda:self.move_image("Images_Family"))
    ...
    def load_menubar(self):
        ...
        upload_music.add_command(label="Cargar Biblioteca de Música",command=lambda:self.load_library("Music"))
        upload_music.add_separator()
        upload_music.add_command(label="Cargar Carpeta de Música a la Biblioteca",command=lambda:self.find_in_folder("Music"))
        upload_music.add_separator()
        upload_music.add_command(label="Agregar Archivo/s de Música a la Biblioteca",command=lambda:self.add_files_to_db("Music"))
        upload_music.add_separator()
        upload_music.add_command(label="Eliminar Archivos de Música de la Biblioteca",command=lambda:self.clear_database("Music"))
        music_menu.add_cascade(label='Biblioteca de Música',menu=upload_music)
        favorite_music=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
        favorite_music.add_command(label="Cargar Biblioteca de Música de Favoritas",command=lambda:self.load_library("Music_Favorite"))
        favorite_music.add_separator()
        favorite_music.add_command(label="Cargar Carpeta de Música Favorita a la Biblioteca",command=lambda:self.find_in_folder("Music_Favorite"))
        favorite_music.add_separator()
        favorite_music.add_command(label="Agregar Archivo/s de Música Favoritos a la Biblioteca",command=lambda:self.add_files_to_db("Music_Favorite"))
        favorite_music.add_separator()
        favorite_music.add_command(label="Eliminar Archivos de Música Favoritos de la Biblioteca",command=lambda:self.clear_database("Music_Favorite"))
        music_menu.add_cascade(label="Biblioteca de Música Favorita",menu=favorite_music)
        download_music=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
        download_music.add_command(label="Descargar Música de Youtube a la Carpeta",command=lambda:self.download_from_youtube("Music"))
        music_menu.add_cascade(label='Descargar Música de Youtube',menu=download_music)
        music_menu.add_separator()
        upload_videos=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
        upload_videos.add_command(label="Cargar Biblioteca de Videos",command=lambda:self.load_library("Videos"))
        upload_videos.add_separator()
        upload_videos.add_command(label="Cargar Carpeta de Video a la Biblioteca",command=lambda:self.find_in_folder("Videos"))
        upload_videos.add_separator()
        upload_videos.add_command(label="Agregar Archivo/s de Vídeo a la Biblioteca",command=lambda:self.add_files_to_db("Videos"))
        upload_videos.add_separator()
        upload_videos.add_command(label="Eliminar Archivos de Video de la Biblioteca",command=lambda:self.clear_database("Videos"))
        ...
        favorite_videos.add_command(label="Cargar Biblioteca de Videos Favoritos",command=lambda:self.load_library("Videos_Favorite"))
        favorite_videos.add_separator()
        favorite_videos.add_command(label="Cargar Carpeta de Video Favorita a la Biblioteca",command=lambda:self.find_in_folder("Videos_Favorite"))
        favorite_videos.add_separator()
        favorite_videos.add_command(label="Agregar Archivo/s de Video Favoritos a la Biblioteca",command=lambda:self.add_files_to_db("Videos_Favorite"))
        favorite_videos.add_separator()
        favorite_videos.add_command(label="Eliminar Archivos de Video Favoritos de la Biblioteca",command=lambda:self.clear_database("Videos_Favorite"))
        music_menu.add_cascade(label='Biblioteca de Videos Favoritos',menu=favorite_videos)
        karoake_videos=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
        karoake_videos.add_command(label="Cargar Biblioteca de Videos de Karaoke",command=lambda:self.load_library("Videos_Karaoke"))
        karoake_videos.add_separator()
        karoake_videos.add_command(label="Cargar Carpeta de Video de Karaoke a la Biblioteca",command=lambda:self.find_in_folder("Videos_Karaoke"))
        karoake_videos.add_separator()
        karoake_videos.add_command(label="Agregar Archivo/s de Video de Karaoke a la Biblioteca",command=lambda:self.add_files_to_db("Videos_Karaoke"))
        karoake_videos.add_separator()
        karoake_videos.add_command(label="Eliminar Archivos de Video de Karaoke de la Biblioteca",command=lambda:self.clear_database("Videos_Karaoke"))
        music_menu.add_cascade(label='Videoteca de karaoke',menu=karoake_videos)
        download_menu=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
        download_menu.add_command(label="Descargar Video de Youtube a la Carpeta",command=lambda:self.download_from_youtube("Video"))
        music_menu.add_cascade(label='Descargar Video de Youtube',menu=download_menu)
        music_menu.add_separator()
        upload_image=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
        upload_image.add_command(label="Cargar Biblioteca de Imágenes",command=lambda:self.load_library("Images"))
        upload_image.add_separator()
        upload_image.add_command(label="Cargar Carpeta de Imágenes a la Biblioteca",command=lambda:self.find_in_folder("Images"))
        upload_image.add_separator()
        upload_image.add_command(label="Agregar Archivo/s de Imagen a la Biblioteca",command=lambda:self.add_files_to_db("Images"))
        upload_image.add_separator()
        upload_image.add_command(label="Eliminar Archivos de Imagen de la Biblioteca",command=lambda:self.clear_database("Images"))
        music_menu.add_cascade(label='Biblioteca de Imágenes',menu=upload_image)
        family_image=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
        family_image.add_command(label="Cargar Biblioteca de Imágenes de la Familia",command=lambda:self.load_library("Images_Family"))
        family_image.add_separator()
        family_image.add_command(label="Cargar Carpeta de Imágenes Familiares a la Biblioteca",command=lambda:self.find_in_folder("Images_Family"))
        family_image.add_separator()
        family_image.add_command(label="Agregar Archivo/s de Imagen Familiar a la Biblioteca",command=lambda:self.add_files_to_db("Images_Family"))
        family_image.add_separator()
        family_image.add_command(label="Eliminar Archivos de Imagen Familiar de la Biblioteca",command=lambda:self.clear_database("Images_Family"))
        music_menu.add_cascade(label='Biblioteca de Imágenes Familiares',menu=family_image)
        favorite_image=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
        favorite_image.add_command(label="Cargar Biblioteca de Imágenes de Favoritos",command=lambda:self.load_library("Images_Favorite"))
        favorite_image.add_separator()
        favorite_image.add_command(label="Cargar Carpeta de Imágenes Favoritas a la Biblioteca",command=lambda:self.find_in_folder("Images_Favorite"))
        favorite_image.add_separator()
        favorite_image.add_command(label="Agregar Archivo/s de imagen favoritos a la biblioteca",command=lambda:self.add_files_to_db("Images_Favorite"))
        favorite_image.add_separator()
        favorite_image.add_command(label="Eliminar Archivos de Imagen Favoritos de la Biblioteca",command=lambda:self.clear_database("Images_Favorite"))
        music_menu.add_cascade(label='Biblioteca de Imágenes Favoritas',menu=favorite_image)
        music_menu.add_command(label="Presentación de Diapositivas de Imágenes",command=lambda:set_slide_show())
        ...
        if os.path.isfile(soundview_path):
            ...
            self.menubar.add_cascade(label=' Seleccione el Dispositivo de Salida de Audio ',menu=device_menu)
            ...
            svv.add_command(label="Ver / Configurar Todos los Dispositivos",command=lambda:open_soundview())
    ...
    def download_from_youtube(self,db):
        home_dir=Path.home()
        save_path=os.path.join(home_dir,f"Descargas de Youtube ({db})")
        title="<Descargas de Youtube>"
        msg1='Ingrese una URL de Youtube para Descargar. Para Obtene\n'
        msg2=f'una URL, abra su Archivo {db} en Youtube y comience a \n'
        msg3=f'jugar. Haga clic Derecho en {db} y Seleccione Copiar URL.\n'
        msg4='Pega la URL en el Campo de Abajo.'        
        msg=msg1+msg2+msg3+msg4
        my_url=""
        link=my_askstring(title,msg,my_url)
        if link==None:return
        if not os.path.exists(save_path):
            msg1=f"El archivo se Guardará en {save_path}"
        ...
        try:
            ...
            return
        title="<Descargas de Youtub>"
        msg=f'La Descarga a {save_path} se Realizó Correctamente.\n'
        messagebox.showinfo(title, msg)
    ...
    def load_library(self,db,skip=None):
        ...
        if len(self.Media_Dict)==0:
            ...
            msg=msg1+msg2
            messagebox.showwarning(f"<{self.active_database} Biblioteca>",msg)
            return
        root.title(f"Mi Reproductor Multimedia 4.8 ({self.active_database} Biblioteca)")
...
def set_slide_show():
    ...
    msg1='Introduzca un Tiempo de Retardo en Segundos para la\n'
    msg2='Presentación de Diapositivas de Imágenes. Un Tiempo de\n'
    msg3='Retardo de 0 Segundos Indica que no hay Presentación de\n'
    msg4='Diapositivas. El Tiempo Máximo de Retardo es de 120 Segundos.'
    msg=msg1+msg2+msg3+msg4
...
if __name__ == '__main__':
    ...
    root.title("Mi Reproductor Multimedia 4.8")
    ...
    volume_lbl=tk.Button(title_frame,text='Volumen Maestro',image=btn_skin, compound="center",width=pix_wid+2,height=20,activeforeground='#4cffff',
