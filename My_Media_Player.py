    ...
    def play(self,file,key):
        if self.next_ready:
            ...
            if not self.ffplay_running:# Not Running
                ...
                self._reset_timer()
                if self.active_media=="image":# Play Images
                    self.play_images(file,key)
                    return
                else:# Play Music And Videos
                    ...
                    try:
                        ...
                        self.parent.canvas.update()
                        self.process_ffplay=subprocess.Popen(["ffplay","-ss",str(self._start_time),"-t",str(self._duration),"-x",video_width,"-y", video_height,"-autoexit",file,"-window_title", window_title],
