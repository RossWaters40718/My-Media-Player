...
class FFPLAY():
                ...
                if self.active_media=="image":
                    try:
                        ...
                        if int(Slide_Show_Delay.get())>0: 
                            ...
                            self.Media_Btn[key].configure(fg="#ffffff")
                            self.Media_Btn[key].update()    
                            if key!=None:
                                if key==len(self.Media_Dict)-1:
                                    file=self.Media_Dict["0"]
                                    key=0
                                else:    
                                    key+=1
                                    file=self.Media_Dict[str(key)]
                            root.update()        
    ...
    def play(self,file,key):
        if self.next_ready:
            ...
            self.active_file=file
            ext=str(os.path.splitext(basename)[1].replace(".",""))
            if ext.lower() in self.ffmpeg_image_exts:self.active_media="image"
            elif ext.lower() in self.ffmpeg_audio_exts:self.active_media="music"
            elif ext.lower() in self.ffmpeg_video_exts:self.active_media="video"
            else:return    
            if not self.ffplay_running:# Not Running
                ...
                video_height=Screen_Height.get()
                if self.active_media=="image":
                    self.play_images(file,key)
                    return
                limit=sys.getrecursionlimit()
                if limit==1000:sys.setrecursionlimit(1500)                
                video_width=str(int(int(Screen_Height.get())*aspect_ratio))    
                if int(video_width)>work_area[2]:video_width=str(work_area[2])
                if int(video_height)>work_area[3]:video_height=str(work_area[3])
                if Screen_Position.get()=="Top Left":
                    video_x,video_y=0,0
    ...
    def ctrl_btn_clicked(self,event,btn):
        if self.Media_Dict:
            if btn=="btn play":# If Playing, Do Nothing
                if self.cv2_running and int(Slide_Show_Delay.get())>0:self.listener.stop()
                if self.ffplay_running or self.cv2_running:self.stop_process()
                if self.shuffled:
                    self.shuffled=False
