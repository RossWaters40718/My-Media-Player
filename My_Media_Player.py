...
class FFPLAY():
    def __init__(self,parent):
        ...
        self.video_karaoke_path=os.path.join(Path(__file__).parent.absolute(),"Videos_Karaoke.json")
    ...
    def youtube_downloader(self):
        root.iconify()
        try:
            subprocess.run(['youtube_GUI_Downloader.exe'])
        ...
        if db!="" and path!="" and file_name!="":
            ...
            root.update()
            messagebox.showinfo("< YouTube Downloader >", f'{file_name} Was Added To {db} Library')
        elif path!="" and file_name!="":      
            messagebox.showinfo("< YouTube Downloader >", f'{file_name} Was Added To {path}')
        else:messagebox.showwarning("< YouTube Downloader >", "No File Or Path Detected!")    
    ...
    def load_menubar(self):
        ...
        downloader_path=os.path.join(Path(__file__).parent.absolute(),"youtube_GUI_Downloader.exe")
        if os.path.exists(downloader_path):
            download_menu=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
            self.menubar.add_cascade(label=' Downloader ',menu=download_menu)
    ...
    def load_library(self,db,skip=None):
        ...
        if len(self.Media_Dict)==0:
            ...
            root.title(f"My Media Player 5.6 ({db.replace("_"," ")} Library), Playing On Audio Device: {self.Active_Device}")
    ...
    def select_output_device(self,device):
        try:
            ...
            Level.set(5)
            self.Master_Volume.SetMasterVolumeLevelScalar(Level.get()/ 100, None)
            root.title(f"My Media Player 5.6 ({self.active_database.replace("_"," ")} Library), Playing On Audio Device: {self.Active_Device}")
        except Exception as ex:
...
if __name__ == '__main__':
    ...
    root.title("My Media Player 5.6")
