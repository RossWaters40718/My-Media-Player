...
class FFPLAY():
    ...
    def begin_seeking(self,event):
        if self.ffplay_running:
            ...
            if clicked=="trough1":
                ...
                self.ctrl_btn_clicked(self,"pause")
                self.seeking=True
        ...
        if self.ffplay_running:
            ...
            if unclicked=="slider" or unclicked=="": 
                ...
                if ext.lower() in self.ffmpeg_image_exts:
                    time_scale.set(0.0)
                    self.start_time=time_scale.get()
                    return# Image File
    ...
    def play(self,file,key):
        if self.next_ready:
            ...
            if ext.lower() in self.ffmpeg_image_exts:
                ...
                self.num_frames=0
                self.fps=0
            elif ext.lower() in self.ffmpeg_audio_exts:
            ...
            if not self.ffplay_running:# Not Running
                ...
                if self.active_media!="image" and self.start_time==0.0:
                ...
                try:
                    ...
                    if self.active_media=="image":
                        ...
                        if int(Slide_Show_Delay.get())>0:
                            time.sleep(int(Slide_Show_Delay.get()))
                            self.ctrl_btn_clicked(self,"next")
                    else:
                        self.process_ffplay=subprocess.Popen(["ffplay","-ss",str(self.start_time),"-t",str(self._duration),"-x",video_width,"-y", video_height,"-autoexit",file,"-window_title", window_title],
    ...
    def _reset_timer(self):        
        Start_Time.set(0.0)
        time_scale.set(self.start_time)
        time_scale.update()
        self._interval=0.1
    ...
    def rotate_image(self):
        ...
        if angle!=None:
            try:
                ...
                msg2=f"Error: '{e}'"
                messagebox.showerror(title, msg1+msg2)
    def remove_media_file(self,show_msg=None):
        ...
        if status:
            ...
            elif self.active_database=="Music_Favorite":db_path=self.music_favorite_pathq
    ...
    def load_menubar(self):
        ...
        slide_show_image=Menu(self.menubar,background='aqua',foreground='black',tearoff=0) # SubMenu
        slide_show_image.add_command(label="Delay Time",command=lambda:set_slide_show())
        music_menu.add_cascade(label='Image Slide Show',menu=slide_show_image)
