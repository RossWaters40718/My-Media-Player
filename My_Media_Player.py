...
class FFPLAY():
    ...
    def load_menubar(self):
        ...
        download_music.add_command(label="Download Youtube Music To Folder",command=lambda:self.download_from_youtube("Music"))
        music_menu.add_cascade(label='Download Youtube Music',menu=download_music)
        ...
        karoake_videos.add_command(label="Load Karaoke Video Library",command=lambda:self.load_library("Videos_Karaoke"))
        karoake_videos.add_separator()
        karoake_videos.add_command(label="Upload Karaoke Video Folder To Library",command=lambda:self.find_in_folder("Videos_Karaoke"))
        karoake_videos.add_separator()
        karoake_videos.add_command(label="Upload Karaoke Video File/s To Library",command=lambda:self.add_files_to_db("Videos_Karaoke"))
        karoake_videos.add_separator()
        karoake_videos.add_command(label="Remove Karaoke Video Files From Library",command=lambda:self.clear_database("Videos_Karaoke"))
        music_menu.add_cascade(label='Karaoke Video Library',menu=karoake_videos)
        download_menu=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)
        download_menu.add_command(label="Download Youtube Video To Folder",command=lambda:self.download_from_youtube("Video"))
        music_menu.add_cascade(label='Download Youtube Video',menu=download_menu)
        music_menu.add_separator()
    ...
    def download_from_youtube(self,db):
        home_dir=Path.home()
        save_path=os.path.join(home_dir,f"Youtube Downloads ({db})")
        title="<Youtube Downloads>"
        msg1='Please Enter A Youtube URL For Downloading. To Obtain\n'
        msg2=f'A URL, Open Your {db} File In Youtube And Start Play.\n'
        msg3='Right Click On The Play Window And Select Copy URL.\n'
        msg4='Then Paste The Copied URL In The Field Below.'
        msg=msg1+msg2+msg3+msg4
        my_url=""
        link=my_askstring(title,msg,my_url)
        if link==None:return
        if not os.path.exists(save_path):
            msg1=f"The File Will Be Saved To {save_path}"
        ...
        try:
            ...
            return
        title="<Youtube Downloads>"
        msg=f'Download To {save_path} Was Successful.\n'
        messagebox.showinfo(title, msg)
