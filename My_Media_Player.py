...
class FFPLAY():
    def __init__(self,parent):
        ...
        self.Media_Dict={}# Song,Video,Image Dictionary
        self.active_database=""
        self.active_file=""
        self.Index=[]# Index Widgets
        self.Media_Btn=[]# Media Widgets
        self.File_Count=0# Media Widget Count
        ...
        self.screen_path=os.path.join(Path(__file__).parent.absolute(),"Screen.json")
    ...
    def get_duration(self,file):# minutes = "-sexagesimal", seconds = Blank
        try:
            ...
            if ext.lower() in self.ffmpeg_image_exts:return 
            if ext=="webm":# Format Duration
                proc=subprocess.Popen(["ffprobe","-v","error","-show_entries","format=duration","-of","default=noprint_wrappers=1:nokey=1",file], 
                                        stdout=subprocess.PIPE,creationflags=subprocess.CREATE_NO_WINDOW, stderr=subprocess.PIPE)
            else:# Stream Duration
            ...
            proc.terminate()
            if proc.returncode!=0:raise Exception("ffprobe Get Strean Duration")
            else:
    ...
    def play(self,file,key):
        if self.next_ready:
            ...
            self.active_file=""
            ext=str(os.path.splitext(basename)[1].replace(".",""))
            if ext.lower() in self.ffmpeg_image_exts:images=True # Image
            else:images=False
            if not self.ffplay_running:# Not Running
                ...
                self._reset_timer()
                video_height=Screen_Height.get()
                if images:
                    try:
                        if not self.cv2_running:self.load_image_menu()
                        img=cv2.imread(file,cv2.IMREAD_UNCHANGED)
                        self.active_file=file
                        hgt, wid=img.shape[:2]
                        img_aspect_ratio=wid/hgt
                        if hgt>int(video_height):hgt=int(video_height)
                        scale_factor=int(video_height)/hgt  # Percent of original size
                        video_height=int(hgt * scale_factor)
                        if video_height<hgt:video_height=hgt
                        video_width=int(video_height * img_aspect_ratio)
                    except Exception as e:
                        pass
                        self.delete_image(False)
                        return
                else:video_width=str(int(int(Screen_Height.get())*aspect_ratio))    
                if int(video_width)>work_area[2]:video_width=str(work_area[2])
                if int(video_height)>work_area[3]:video_height=str(work_area[3])
                if Screen_Position.get()=="Top Left":
                    video_x,video_y=0,0
                elif Screen_Position.get()=="Top Center":
                    video_x,video_y=int((work_area[2]/2)-(int(video_width)/2)),0
                elif Screen_Position.get()=="Top Right":
                    video_x,video_y=work_area[2]-int(video_width),0
                elif Screen_Position.get()=="Center Left":
                    video_x,video_y=0,int((work_area[3]/2)-(int(video_height)/2))
                elif Screen_Position.get()=="Center":
                    video_x,video_y=int((work_area[2]/2)-(int(video_width)/2)),int((work_area[3]/2)-(int(video_height)/2))
                elif Screen_Position.get()=="Center Right":
                    video_x,video_y=int((work_area[2])-(int(video_width))),int((work_area[3]/2)-(int(video_height)/2))
                elif Screen_Position.get()=="Bottom Left":
                    video_x,video_y=0,work_area[3]-(int(video_height))
                elif Screen_Position.get()=="Bottom Center":
                    video_x,video_y=int((work_area[2]/2)-(int(video_width)/2)),work_area[3]-(int(video_height))
                elif Screen_Position.get()=="Bottom Right":
                    video_x,video_y=int((work_area[2])-(int(video_width))),work_area[3]-(int(video_height))
                try:
                    ...
                    if images:
                        ...
                        win32gui.MoveWindow(handle, video_x, video_y, video_width, video_height, 1)
                        cv2.waitKey(1)
                        ...
                        self.process_ffplay=subprocess.Popen(["ffplay","-x",video_width,"-y", video_height,"-autoexit",file,"-window_title", window_title],
                                                            stdin=subprocess.PIPE,stdout=subprocess.PIPE,creationflags=subprocess.CREATE_NO_WINDOW)
                        Start_Time.set(perf_counter_ns())
                        time.sleep(0.5)# Wait for the window to start, Computer Dependent
                        ...
                        if poll==None:# None = ffplay Running
                            ...
                            ready=False
                            count=0# Exit Backup
                            while ready==False and count<=30:# 30 = 3 Seconds Max Time For Loop
                                count+=1
                                try:
                                    time.sleep(0.1)
                                    self.ffplay_window=window.getWindowsWithTitle(window_title)[0]# Window
                                    ...
                                    pass
                            if count>=30:raise Exception("getWindowsWithTitle()")        
                            handle=win32gui.FindWindow(None, window_title)# Window Handle
                            win32gui.MoveWindow(handle, video_x, video_y, int(video_width), int(video_height), 1)
                    ...
                    messagebox.showerror(title, msg1+msg2)
                    self._stop()
    def play_explorer_file(self,file):
        basename=os.path.basename(file)
        ext=str(os.path.splitext(basename)[1].replace(".",""))
        if ext.lower() in self.ffmpeg_audio_exts:
            self.add_files_to_db("Music",file)
            self.load_library("Music")
        elif ext.lower() in self.ffmpeg_video_exts:
            self.add_files_to_db("Videos",file)
            self.load_library("Videos")
        for key, val in self.Media_Dict.items():
                if val==file:break
        self.key_now=int(key)        
        self.play(file,self.key_now)       
    ...
    def _update_timer(self):
        ...
        self._start_timer_thread()
    def slider_trough_clicked(self,event):
        if self.ffplay_running:
    ...
    def send_keyboard_key(self,key):
        keyboard=Controller()
        mykeys=[Key.left,Key.right,Key.up,Key.down,"p","q"]
        if self.ffplay_running:
    ...
    def ctrl_btn_clicked(self,event,btn):
        if self.Media_Dict:
            if btn=="btn play":# If Playing, Do Nothing
                ...
                self.play(file,self.key_now)
            elif btn=="shuffled":# File Clicked In Window
                if not self.ffplay_running: 
                    if not self.shuffled: 
                        self.shuffled=True
                        self.destroy_widgets()
                        self.load_library(self.active_database)
                ...
                if self.next_ready:# Prevent Multiple Windows
                    if self.ffplay_running or self.cv2_running:
                        ...
                        if self.last_key!=None:
                            if self.last_key==len(self.Media_Dict)-1:
                                file=self.Media_Dict["0"]
                                self.key_now=0
                            else:    
                                self.key_now=self.last_key+1    
    ...
    def rotate_image(self):
        ...
        if angle!=None:
            try:
                ...
                self.play(self.active_file,self.key_now)
            except Exception as e:
                title='<Image Rotation>'
                ...
                msg2=f"Error: '{e}'"
                messagebox.showerror(title, msg1+msg2)
    def delete_image(self,show_msg=None):
        if show_msg==None:# Ask To Delete Image
            title="<Delete Image>"
            msg1="Are You Sure You Want To Delete\n"
            msg2=f"{self.active_file}?"
            msg=msg1+msg2
            status=messagebox.askyesno(title, msg)
        else:status=True# Don't Ask, Auto Delete Corrupted Images    
        if status:
    ...
    def load_image_menu(self):
        ...
        images_menu.add_command(label='Delete Image',command=lambda:self.delete_image())
    ...
    def load_menubar(self):
        ...
        upload_music=Menu(self.menubar,background='aqua',foreground='black',tearoff=0) # SubMenu
        ...
        favorite_music=Menu(self.menubar,background='aqua',foreground='black',tearoff=0) # SubMenu
        ...
        upload_videos=Menu(self.menubar,background='aqua',foreground='black',tearoff=0) # SubMenu
        ...
        favorite_videos=Menu(self.menubar,background='aqua',foreground='black',tearoff=0) # SubMenu
        ...
        karoake_videos=Menu(self.menubar,background='aqua',foreground='black',tearoff=0) # SubMenu
        ...
        music_menu.add_separator()
        upload_image=Menu(self.menubar,background='aqua',foreground='black',tearoff=0) # SubMenu
        ...
        family_image=Menu(self.menubar,background='aqua',foreground='black',tearoff=0) # SubMenu
        ...
        favorite_image=Menu(self.menubar,background='aqua',foreground='black',tearoff=0) # SubMenu
        ...
        screen_menu=Menu(self.menubar,background='aqua',foreground='black',tearoff=0)# File Menu and commands
    ...
    def write_screen_setup(self):
        temp_dict={}
        sc=json.load(open(self.screen_path, "r"))
        json.dump(sc,open(self.screen_path, "w"),indent=4)
        temp_dict[0]=Screen_Height.get()
        temp_dict[1]=Screen_Position.get()
        with open(self.screen_path, "w") as outfile:json.dump(temp_dict, outfile)
        outfile.close()
        temp_dict.clear()
    def load_screen_setup(self):
        temp_dict=json.load(open(self.screen_path, "r+"))
        if len(temp_dict)==0:
            hgt=str(screen_height-root_height)
    ...
    def load_library(self,db,skip=None):
        if skip==None:self.load_screen_setup()
        if db=="Music":path=self.music_path
        elif db=="Music_Favorite":path=self.music_favorite_path
        elif db=="Videos":path=self.video_path
        elif db=="Videos_Favorite":path=self.video_favorite_path
        ...
        for key,self.file in self.Media_Dict.items():
            self.Index.append(self.File_Count)
            self.Media_Btn.append(self.File_Count)
            self.Index[self.File_Count]=Label(self.parent.media_window,text=str(self.File_Count+1)+".",relief="flat",
                        anchor='w',justify="left",background="#001829",foreground="#ffffff",font=media_font,)
            self.Index_Widgets[self.File_Count]=self.Index[self.File_Count]
            self.Index[self.File_Count].grid(row=self.File_Count,column=0,columnspan=1,sticky=W)
            self.Media_Btn[self.File_Count]=Button(self.parent.media_window,name=str(self.File_Count),bg="#001829",fg="#ffffff", font=media_font,
                        anchor=NW,justify='center',relief='flat',border=None)
            self.Media_Btn[self.File_Count].bind("<ButtonRelease>",lambda event:self.ctrl_btn_clicked(event,"media play"))
            basename=os.path.basename(self.Media_Dict[key])
            text=os.path.splitext(basename)[0]
            self.Media_Btn[self.File_Count].configure(text=text,activebackground="#4cffff")
            self.Media_Widgets[self.File_Count]=self.Media_Btn[self.File_Count]
            self.Media_Btn[self.File_Count].grid(row=self.File_Count,column=1,columnspan=1,sticky=W)
            if self.File_Count>1200:break 
            self.File_Count+=1
...
def set_screen_size():
    ...
    default=str(screen_height-root_height)
    ...
    msg=msg1+msg2+msg3+msg4
    hgt=my_askinteger(title,msg,int(Screen_Height.get()),100,work_area[3])
    if hgt!=None:
        Screen_Height.set(str(hgt))
        FF_Player.write_screen_setup()
...
def destroy(restart=None):
    ...
    os._exit(0)
def get_clicked_mediafile():
    # This Application Must Be Selected As Default Media Player For The Desired Extensions.
    # This Functions Only Searches In User/Music And User/Videos Folders
    # For Recently Clicked Files In Windows Explorer.
    folders=['Music','Videos']
    recent_file=""
    now_time=time.time()
    for f in range(len(folders)):
        if f==0:exts=['mp3','wav','wma','mp2','ac3','aac','eac3','m4a','wmav1','wmav2','opus','ogg','aiff','alac','ape','flac']
        elif f==1:exts=['mp4','avi','mov','mkv','mpg','mpeg','wmv','webm','flv','mj2','3gp','3g2']
        try:
            for e in range(len(exts)):# User/Music User/Videos Folders
                media_files=glob.glob(os.path.join(os.path.expanduser("~"),folders[f],'**','*'+exts[e]), recursive=True)
                last_media_file=max(media_files, key=os.path.getatime)
                last_media_path=os.path.abspath(last_media_file)
                timestamp=os.path.getatime(Path(last_media_path))
                if now_time-timestamp<8:# Allow 8 Sec. Delay
                    recent_file=last_media_path.replace("\\","/")
                    break
            if recent_file!="":break
        except Exception:pass
    exts.clear()    
    if recent_file=="":
        return None# No Recent Files Selected In File Explorer Within 10 Seconds
    else:
        try:# Close File Explorer If Still Opened
            [process.kill() for process in psutil.process_iter() if process.name() == "explorer.exe"]    
        except Exception:pass
        return recent_file# Return Selected Song From File Explorer For Playing
if __name__ == '__main__':
    root=tk.Tk()
    explorer_file_clicked=get_clicked_mediafile()# Application Started By Media File Executed In File Explorer.
    style=ttk.Style()
    ...
    time_scale.bind("<ButtonRelease-1>",lambda event:FF_Player.slider_trough_clicked(event))
    ...
    pause_btn.bind("<ButtonRelease>",lambda event:FF_Player.ctrl_btn_clicked(event,"pause"))
    ...
    root.bind("<ButtonRelease-1>",lambda event:FF_Player.slider_released())# Sets Video Window Active
    ...
    root.update()
    if explorer_file_clicked!=None:FF_Player.play_explorer_file(explorer_file_clicked)
    else:root.after(500, FF_Player.load_library("Music"))    
    root.mainloop()
