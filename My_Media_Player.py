...
class FFPLAY():
    ...
    def load_menubar(self):
        ...
        if os.path.isfile(soundview_path):
            ...
            self.menubar.add_cascade(label=' Select Audio Output Device ',menu=self.device_menu)
            self.update_devices()
    ...
    def on_complete(self,path,name):
        ...
        msg2=f"File Saved To {path}.\n"
        msg3=f"{name} Was Added To {self.youtube_db} Library."
        msg=msg1+msg2+msg3
        messagebox.showinfo(title,msg)
        self.youtube_db=""
    def download_from_youtube(self,type):
        ...
        if not os.path.exists(yt_folder):
            msg1=f"The {type} File Will Be Saved To {yt_folder}"
        ...
        msg5="Next, Paste The Copied URL In The Field Below Using\n"
        msg6="Keyboard Keys ctrl+v."
    ...
    def load_library(self,db,skip=None):
        ...
        if len(self.Media_Dict)==0:
            ...
            root.title(f"My Media Player 5.4 ({db.replace("_"," ")} Library), Playing On Audio Device: {self.Active_Device}")
    ...
    def select_output_device(self,device):
        try:
            ...
            Level.set(5)
            self.Master_Volume.SetMasterVolumeLevelScalar(Level.get()/ 100, None)
            root.title(f"My Media Player 5.4 ({self.active_database.replace("_"," ")} Library), Playing On Audio Device: {self.Active_Device}")
        except Exception as ex:
...
if __name__ == '__main__':
    ...
    root.title("My Media Player 5.4")
